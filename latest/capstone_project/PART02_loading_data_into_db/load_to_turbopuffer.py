        ...
        with Progress() as progress:
            ...
            for conversation in conversation_stream:
                ...
                message_text = conversation[
                    "first_message"
                ]  # we dont truncate the message, only truncate for embeddings
                ...
                if len(batch_data["id"]) >= batch_size:
                    try:
                        ...
                        embeddings = create_embeddings(
                            [
                                text[:2000]
                                for text in batch_texts  # truncate the text to 2000 characters for embeddings
                            ]
                        )
        ...
        try:
            count_result = ns.query(
                ...
                filters=(
                    ...
                    "Eq",
                    filter_language,
                ),  # Count documents with our language filter
            )
        ...
        if total_added > 0:
            ...
            try:
                ...
                try:
                    ...
                    console.print(
                        f"Success: Namespace accessible - found {simple_result.aggregations['sample_count']} documents",
                        style="green",
                    )
...
def main(
    namespace_name: str = typer.Option(
        "wildchat_2k", "--namespace-name", help="Name of the Turbopuffer namespace"
    ),
    limit: int = typer.Option(
        2000, "--limit", help="Maximum number of records to load"
    ),
    batch_size: int = typer.Option(
        25, "--batch-size", help="Batch size for writing to Turbopuffer"
    ),
