...
# Page config
st.set_page_config(page_title="Conversation Search", page_icon="🔍", layout="wide")
...
async def perform_search(query: str, backend: str, search_type: str, top_k: int):
    ...
    try:
        ...
        request = SearchRequest(query=query, top_k=top_k, search_type=search_type_enum)
...
def main():
    ...
    st.markdown(
        "Search for similar conversations using vector similarity or full-text search"
    )
    ...
    with col1:
        ...
        if st.button("🔍 Search", type="primary", disabled=not query):
            with st.spinner(f"Searching with {backend}..."):
                # Run async search
                results = asyncio.run(
                    perform_search(query, backend, search_type, top_k)
                )

                if results:
                    st.session_state.search_results = results
                    st.success(
                        f"Found {len(results.results)} results in {results.query_time_ms:.2f}ms"
                    )
        ...
        if st.session_state.search_results:
            ...
            for i, result in enumerate(st.session_state.search_results.results):
                ...
                has_synthetic = (
                    bool(get_synthetic_queries(conv_hash)) if conv_hash else False
                )
                ...
                with st.expander(title):
                    ...
                    with col_a:
                        st.caption(
                            f"**Language:** {result.metadata.get('language', 'N/A')}"
                        )
                        st.caption(
                            f"**Model:** {result.metadata.get('model_name', 'N/A')}"
                        )
                    with col_b:
                        st.caption(
                        ...
                        st.caption(
                            f"**Turns:** {result.metadata.get('conversation_length', 'N/A')}"
                        )
    ...
    with col2:
        ...
        if st.session_state.selected_conversation:
            ...
            # Display metadata
            st.markdown(
                f"**Score:** {result.score:.4f} | **Language:** {result.metadata.get('language', 'N/A')} | **Model:** {result.metadata.get('model_name', 'N/A')}"
            )

            # Check for synthetic queries
            conversation_hash = result.metadata.get("hash", "")
            synthetic_queries = (
                get_synthetic_queries(conversation_hash) if conversation_hash else []
            )

            if synthetic_queries:
                with st.expander(
                    f"🔍 Synthetic Queries Generated ({len(synthetic_queries)})"
                ):
            ...
            with st.expander("Debug: Raw Conversation Data"):
                st.code(
                    result.conversation_string[:1000] + "..."
                    if len(result.conversation_string) > 1000
                    else result.conversation_string
                )
