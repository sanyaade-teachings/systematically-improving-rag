...
class GenericCache:
    ...
    @staticmethod
    def make_recall_key(conversation_hash: str, query: str, max_query_length: int = 50) -> str:
        ...
        safe_query = "".join(c if c.isalnum() or c in '-_' else '_' for c in truncated_query)
    ...
    def make_generic_key(*parts: str) -> str:
        ...
        for part in parts:
            ...
            safe_part = "".join(c if c.isalnum() or c in '-_' else '_' for c in str(part))
...
def setup_cache(cache_dir: Path, clear_cache: bool = False) -> GenericCache:
    ...
    if clear_cache:
        ...
        if cache_size > 0:
            console.print(f"[yellow]Found {cache_size} cached items[/yellow]")
