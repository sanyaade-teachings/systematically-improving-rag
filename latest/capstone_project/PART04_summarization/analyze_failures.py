...
def analyze_query_patterns(failed_queries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for item in failed_queries:
        ...
        common_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'about'}
...
def analyze(
    ...
    summary_version: str = typer.Option("v4", help="Summary version (v1, v2, v3, or v4)"),
):
    """Analyze failed queries from recall tests"""
    
    # Load failed queries
    file_path = Path(__file__).parent / f"failed_queries_q{query_version}_s{summary_version}.json"
