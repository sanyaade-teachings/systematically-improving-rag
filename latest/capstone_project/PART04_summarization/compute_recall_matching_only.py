...
def load_matching_v2_queries(limit: int = None) -> List[Tuple[str, str, str]]:
    ...
    console.print(f"[cyan]Found {len(summary_hashes)} conversations with summaries[/cyan]")
...
async def verify_single_query(
    ...
    *,
    ...
    cache_key = GenericCache.make_generic_key(
        "recall_summary_match", dao_type, search_type.name, conversation_hash, query[:50]
    )
...
async def main(
    ...
    console.print("[bold green]Recall Verification - Matching Conversations Only[/bold green]")
    console.print("=" * 50)
    ...
    console.print(f"[green]Found {len(matching_queries)} V2 queries with corresponding summaries[/green]")
    ...
    results_table = Table(title="V2 Queries vs Summary-based Embeddings (Matching Only)")
    ...
    console.print(f"V2 queries with {summary_version} summaries: {metrics.get_recall_at_k(10):.2%} recall@10")
