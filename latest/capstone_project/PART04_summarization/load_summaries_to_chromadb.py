...
def create_chromadb_client(use_cloud: bool = True):
    ...
    if use_cloud:
        ...
        if missing_vars:
            raise ValueError(
                f"Missing required environment variables for ChromaDB cloud: {missing_vars}"
            )
...
def load_summaries_to_chromadb(
    ...
    console.print(
        f"Starting summary load to collection: {collection_name}", style="blue"
    )
    ...
    try:
        ...
        if total_added > 0:
            ...
            try:
                ...
                console.print(
                    f"Success: Query found {len(results['documents'][0])} similar summaries",
                    style="green",
                )
                if results["documents"][0]:
                    console.print(
                        f"   Most similar: '{results['documents'][0][0][:100]}...'"
                    )
                    console.print(
                        f"   Hash: {results['metadatas'][0][0].get('hash', 'N/A')}"
                    )
...
def load(
    prefix: str = typer.Option(
        "wildchat-synthetic-summaries",
        "--prefix",
        help="Prefix for the ChromaDB collection",
    ),
...
def delete_collection(
    collection_name: str = typer.Option(
        "wildchat-synthetic-summaries-v2",
        "--collection-name",
        help="Name of the ChromaDB collection to delete",
    ),
