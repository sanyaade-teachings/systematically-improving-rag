...
def load_summaries_chromadb(
    ...
    if drop_existing:
        try:
            ...
            console.print(
                f"[yellow]Dropped existing collection: {collection_name}[/yellow]"
            )
        except Exception:
            pass
...
async def load_summaries_turbopuffer(
    ...
    if drop_existing:
        console.print(
            f"[yellow]TurboPuffer doesn't support collection deletion via API[/yellow]"
        )
    ...
    request = SearchRequest(query=test_query, top_k=5, search_type=SearchType.VECTOR)
    ...
    for i, result in enumerate(results.results):
        console.print(
            f"  {i + 1}. Hash: {result.id[:16]}... (score: {result.score:.4f})"
        )
...
def chromadb(
    ...
    collection_name: str = typer.Option(
        None, help="Collection name (default: wildchat_summaries_{version})"
    ),
    drop: bool = typer.Option(
        False, "--drop", help="Drop existing collection before loading"
    ),
...
def turbopuffer(
    ...
    drop: bool = typer.Option(
        False, "--drop", help="Drop existing data before loading"
    ),
    limit: int = typer.Option(None, help="Limit number of summaries to load"),
):
    """Load summaries into TurboPuffer"""
    asyncio.run(
        load_summaries_turbopuffer(
            summary_version=summary_version,
            namespace=namespace,
            drop_existing=drop,
            limit=limit,
...
def all(
    ...
    async def load_all():
        console.print(
            "[bold green]Loading summaries into all vector databases[/bold green]"
        )
        console.print("=" * 50)

        # Load into ChromaDB
        await load_summaries_chromadb(
            summary_version=summary_version, drop_existing=drop, limit=limit
        )
