    ...
    try:
        ...
        if reset_namespace:
            try:
                ns.delete_all()
                console.print(
                    f"Deleted all data from namespace: {namespace_name}", style="yellow"
                )
            except Exception:
        ...
        if total_added > 0:
            ...
            try:
                ...
                console.print(
                    f"Success: Query found {len(results.rows)} similar summaries",
                    style="green",
                )
                if results.rows:
                    console.print(
                        f"   Most similar: '{results.rows[0].summary[:100]}...'"
                    )
...
def load(
    prefix: str = typer.Option(
        ...
        "--prefix",
        help="Prefix for the Turbopuffer namespace",
    ),
...
def delete_namespace(
    namespace_name: str = typer.Option(
        "wildchat-summaries-v2",
        "--namespace-name",
        help="Name of the Turbopuffer namespace to delete",
    ),
    ...
    try:
        ...
        ns.delete_all()
        console.print(
            f"Successfully deleted all data from namespace: {namespace_name}",
            style="green",
        )
