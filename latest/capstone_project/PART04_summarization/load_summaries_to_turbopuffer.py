...
def load_summaries_to_turbopuffer(
    ...
    console.print(f"Using embedding model: sentence-transformers/all-MiniLM-L6-v2", style="cyan")
        ...
        if total_added > 0:
            ...
            try:
                ...
                for i, row in enumerate(results.rows[:3]):
                    console.print(f"  {i+1}. Hash: {row.hash[:16]}... - {row.text[:100]}...")
...
def main(
    ...
    namespace_name: str = typer.Option(None, help="TurboPuffer namespace (default: wildchat-summaries-{version})"),
