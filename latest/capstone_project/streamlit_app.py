...
async def process_conversation(client, conversation_id: str, conversation: Dict[str, Any]) -> SearchQueries:
    ...
    try:
        result = await generate_search_queries_v1(client, conversation_id, conversation)
        return result
...
def display_conversation(conversation: Dict[str, Any], index: int):
    ...
    with st.expander(f"Conversation {index + 1}: {conversation['conversation_hash'][:8]}..."):
        ...
        if process_button and selected_indices:
            ...
            async def process_all_conversations():
                ...
                for i, idx in enumerate(selected_indices):
                    ...
                    status_text.text(f"Processing conversation {i+1}/{len(selected_indices)}: {conversation_id[:8]}...")
