    ...
    async def add(self, documents: List[WildChatDocument]) -> Dict[str, Any]:
        ...
        for doc in documents:
            try:
                ...
                batch_data['timestamp'].append(doc.timestamp.isoformat() if doc.timestamp else "")
        ...
        try:
            if request.search_type == SearchType.VECTOR:
                ...
                results = self.namespace.query(
                    ...
                    top_k=request.top_k,
                    filters=filters,
                    include_attributes=['text', 'conversation_string', 'hash', 'timestamp', 
                                      'language', 'model_name', 'conversation_length', 
                                      'country', 'toxic', 'redacted', 'turn']
                )
        ...
        try:
            ...
            try:
                lang_result = self.namespace.query(
                    aggregate_by={'languages': ('GroupBy', 'language')},
                    top_k=10
                )
            ...
            try:
                model_result = self.namespace.query(
                    aggregate_by={'models': ('GroupBy', 'model_name')},
                    top_k=10
                )
