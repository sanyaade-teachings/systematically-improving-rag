        ...
        if show_progress:
            with Progress() as progress:
                task = progress.add_task(
                    f"Generating {self.model_name} embeddings", total=len(texts)
                )
...
def load_embeddings_from_parquet(
    parquet_path: Path, return_metadata: bool = True
...
async def generate_conversation_embeddings(
    ...
    """
    console.print(
        f"[bold green]Generating embeddings for {len(conversations)} conversations[/bold green]"
    )
    ...
    if skipped > 0:
        console.print(
            f"[yellow]Skipped {skipped} conversations with empty text[/yellow]"
        )
...
async def generate_summary_embeddings(
    ...
    """
    console.print(
        f"[bold green]Generating embeddings for {len(summaries)} summaries[/bold green]"
    )
    ...
    for s in summaries:
        ...
        meta = {
            "id": s[
                "conversation_hash"
            ],  # Use conversation_hash as ChromaDB doc ID (NOT the database ID with suffix!)
    ...
    output_path = (
        output_dir / f"{technique}_{embedding_model.replace('/', '-')}.parquet"
    )
    save_embeddings_to_parquet(embeddings, metadata, output_path, embedding_model)
...
def truncate_text_to_tokens(
    text: str, max_tokens: int = 8000, model: str = "text-embedding-3-large"
) -> str:
    """
...
async def generate_full_conversation_embeddings(
    ...
    """
    console.print(
        f"[bold green]Generating full conversation embeddings for {len(conversations)} conversations[/bold green]"
    )
    ...
    for conv in conversations:
        ...
        if full_text and full_text.strip():  # Skip empty conversations
            ...
            truncated_text = truncate_text_to_tokens(
                full_text, max_tokens, embedding_model
            )
            ...
            meta = {
                ...
                "truncated": str(
                    len(truncated_text) < original_len
    ...
    if truncated > 0:
        console.print(
            f"[yellow]Truncated {truncated} conversations to {max_tokens} tokens[/yellow]"
        )
