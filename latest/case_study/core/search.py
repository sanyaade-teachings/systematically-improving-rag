...
class ChromaSearchEngine:
    ...
    def __init__(
        ...
        self.collection = self.client.get_or_create_collection(
            name=collection_name, metadata={"embedding_model": embedding_model}
        )
        ...
        console.print(
            f"[green]ChromaDB search engine initialized for collection: {collection_name}[/green]"
        )
    ...
    async def search(
        ...
        if results["ids"] and len(results["ids"][0]) > 0:
            for rank, (id_, distance, metadata) in enumerate(
                ...
                result = SearchResult(
                    id=id_, score=float(score), metadata=metadata, rank=rank + 1
                )
    ...
    async def batch_search(
        ...
        results = self.collection.query(
            query_embeddings=query_embeddings_list, n_results=top_k, where=where
        )
...
class VectorSearchEngine:
    ...
    def __init__(
        self, embeddings_path: Path, embedding_model: str = "text-embedding-3-large"
    ):
    ...
    async def search(
        self, query: str, top_k: int = 10, min_score: Optional[float] = None
    ) -> SearchResults:
    ...
    async def batch_search(
        ...
        all_results = await self.engine.batch_search(
            queries, top_k, show_progress=show_progress
        )
